...
namespace AdminServices.Controllers
{
    ...
    public class LoginController : ControllerBase
    {
        ...
        [HttpPost]
        [Route("Login")]
        public IActionResult Login(TblUserDataDetail user)
        {
            IEnumerable<TblUserDataDetail> tblusr = _dbContext.TblUserDataDetails.Where(x => x.UserId == user.UserId && x.PassWord == user.PassWord);
        ...
        [HttpPost]
        [Route("authenticate")]
        public IActionResult Authenticate(TblUserDataDetail tblUserdetail)
        {
            IEnumerable<TblUserDataDetail> tblusr = _dbContext.TblUserDataDetails.Where(x => x.UserId == tblUserdetail.UserId && x.PassWord == tblUserdetail.PassWord);
            if (tblusr.ToList().Count == 0)
            {
                return Unauthorized();
            }
